...
class MoleculeAdminTest < Capybara::Rails::TestCase
  ...
  test 'admin search a molecule' do
    visit molecules_path
  ...
  test 'admin show all molecules' do
    visit molecules_path
  ...
  test 'admin complete create molecule page' do
  ...
  test 'admin cancel create molecule' do
    visit new_molecule_path
  ...
  test 'admin create unvalid molecule' do
    visit new_molecule_path
  ...
  test 'admin create valid molecules' do
    visit new_molecule_path
  ...
  test 'admin edit pubchem molecule' do
    visit molecule_path(@molone)
    assert page.has_link?('Edit', :exact => true)
    assert page.has_link?('Delete', :exact => true)

    assert page.has_link?('Auto Update')
    assert page.has_button?('Request PubChem Update')
    assert page.has_content?('Run an update retrieval for molecular data. Review the results and select the entry you think is correct!')
    assert page.has_content?('Smiles')
